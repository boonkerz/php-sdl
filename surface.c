...
/* {{{ proto void SDL_DestroySurface(SDL_Surface surface)
...
/* {{{ proto SDL_FillSurfaceRect(SDL_Surface surface, SDL_Rect rect, int color)
...
PHP_FUNCTION(SDL_FillSurfaceRect)
{
	...
	RETURN_LONG(SDL_FillSurfaceRect(surface, &rect, (Uint32)color));
}
/* }}} */

/* {{{ proto int SDL_FillSurfaceRects(SDL_Surface surface, SDL_Rect rect, int count, int color)
...
PHP_FUNCTION(SDL_FillSurfaceRects)
{
	...
	if (nb)
	{
		RETVAL_LONG(SDL_FillSurfaceRects(surface, rects, nb, (Uint32)color));
	}
	else
	{
...
/* {{{ proto void SDL_BlitSurface(SDL_Surface src, SDL_rect &srcrect, SDL_Surface dst [, SDL_rect &dstrect])
...
if (z_drect && !(Z_TYPE_P(z_drect)==IS_NULL || zval_to_sdl_rect(z_drect, &drect))) {
	...
	result = SDL_BlitSurface(src, (Z_TYPE_P(z_srect) == IS_NULL ? NULL : &srect),
...
if (z_drect && !(Z_TYPE_P(z_drect)==IS_NULL || zval_to_sdl_rect(z_drect, &drect))) {
	...
	result = SDL_BlitSurfaceScaled(src, (Z_TYPE_P(z_srect) == IS_NULL ? NULL : &srect),
...
/* {{{ proto void SDL_BlitSurfaceUncheckedScaled(SDL_Surface src, SDL_rect &srcrect, SDL_Surface dst , SDL_rect &dstrect)
...
PHP_FUNCTION(SDL_BlitSurfaceUncheckedScaled)
{
	...
	result = SDL_BlitSurfaceUncheckedScaled(src, &srect, dst, &drect, SDL_SCALEMODE_NEAREST);
...
/* {{{ proto int SDL_SetSurfaceColorKey(SDL_Surface src, int flag [, int key ])
...
/* {{{ proto int SDL_GetSurfaceColorKey(SDL_Surface src, int &key)
...
PHP_FUNCTION(SDL_GetSurfaceColorKey)
{
	...
	FETCH_SURFACE(surface, z_surface, 1);
	result = SDL_GetSurfaceColorKey(surface, &key);
	if (result == 0)
	{
		zval_dtor(z_key);
...
/* {{{ proto void SDL_SetSurfaceClipRect(SDL_Surface src, SDL_Rect cliprect)
...
PHP_FUNCTION(SDL_SetSurfaceClipRect)
{
	...
	{
		RETURN_BOOL(SDL_SetSurfaceClipRect(surface, &rect));
	}
...
static const zend_function_entry php_sdl_surface_methods[] = {
	...
	/* non-static methods */
	PHP_FALIAS(Free, SDL_DestroySurface, arginfo_surface_none)
	PHP_FALIAS(FillRect, SDL_FillSurfaceRect, arginfo_SDL_Surface_FillRect)
	PHP_FALIAS(FillRects, SDL_FillSurfaceRects, arginfo_SDL_Surface_FillRects)
	...
	PHP_FALIAS(GetColorKey, SDL_GetSurfaceColorKey, arginfo_SDL_Surface_GetColorKey)
