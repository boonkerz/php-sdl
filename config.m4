...
if test "$PHP_SDL" != "no"; then
  ...
  PHP_ADD_LIBRARY_WITH_PATH(tray, [$SDL_DIR/lib], SDL_SHARED_LIBADD)


  AC_DEFINE(HAVE_SDL, 1, [Whether SDL support is enabled])
  PHP_SUBST(SDL_SHARED_LIBADD)
  SDL_SOURCE_FILES="php_sdl.c \
    version.c sdl.c window.c messagebox.c glcontext.c render.c event.c ttf.c ttf_font.c color.c surface.c rect.c iostream.c image.c systray.c"
